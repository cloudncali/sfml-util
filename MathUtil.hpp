...
namespace MathUtil
{
	...
	template <class T>
	sf::Vector2<T> IndexToCoord(T a_Index, T a_Width)
	{
	...
	template <class T>
	sf::Vector2<T> IndexToCoord(T a_Index, sf::Vector2<T> a_vBounds)
	{
		return IndexToCoord<T>(a_Index, a_vBounds.x);
	}
	//Helper functions
	inline sf::Vector2i IndexToCoord(int a_iIndex, int a_iWidth) { return IndexToCoord<int>(a_iIndex, a_iWidth); }
	inline sf::Vector2f IndexToCoord(float a_fIndex, float a_fWidth) { return IndexToCoord<float>(a_fIndex, a_fWidth); }
	inline sf::Vector2u IndexToCoord(unsigned int a_uIndex, unsigned int a_uWidth) { return IndexToCoord<unsigned int>(a_uIndex, a_uWidth); }

	inline sf::Vector2i IndexToCoord(int a_iIndex, sf::Vector2i a_vBounds) { return IndexToCoord<int>(a_iIndex, a_vBounds.x); }
	inline sf::Vector2f IndexToCoord(float a_fIndex, sf::Vector2f a_vBounds) { return IndexToCoord<float>(a_fIndex, a_vBounds.x); }
	inline sf::Vector2u IndexToCoord(unsigned int a_uIndex, sf::Vector2u a_vBounds) { return IndexToCoord<unsigned int>(a_uIndex, a_vBounds.x); }

	//Take a 2d vector and translate it to a single valued index based off the max width of the area.
	template <class T>
	T CoordToIndex(sf::Vector2<T> a_vVector, T a_Width)
	{
		return a_vVector.x + (a_vVector.y * a_Width);
	}
	template <class T>
	T CoordToIndex(sf::Vector2<T> a_vVector, sf::Vector2<T> a_vBounds)
	{
	...
	//Width
	inline int CoordToIndex(sf::Vector2i a_vVector, int a_iWidth) { return CoordToIndex<int>(a_vVector, a_iWidth); }
	inline float CoordToIndex(sf::Vector2f a_vVector, float a_fWidth) { return CoordToIndex<float>(a_vVector, a_fWidth); }
	inline unsigned int CoordToIndex(sf::Vector2u a_vVector, unsigned int a_uWidth) { return CoordToIndex<unsigned int>(a_vVector, a_uWidth); }
	//Vectors
	inline int CoordToIndex(sf::Vector2i a_vVector, sf::Vector2i a_vBounds) { return CoordToIndex<int>(a_vVector, a_vBounds); }
	inline float CoordToIndex(sf::Vector2f a_vVector, sf::Vector2f a_vBounds) { return CoordToIndex<float>(a_vVector, a_vBounds); }
	inline unsigned int CoordToIndex(sf::Vector2u a_vVector, sf::Vector2u a_vBounds) { return CoordToIndex<unsigned int>(a_vVector, a_vBounds); }
